...
# module = load_module("samplers/uniform_design")
# UniformDesignSampler = module.UniformDesignSampler
importance_results = []

# 目的関数値に基づくPCAの第一主成分軸を描く関数
def plot_pca_direction_by_objective(sampled_points, objective_values, ax, color="blue"):
...
for theta in np.arange(0, 180, 10):  # 0度から180度まで回転
    ...
    plot_pca_direction_by_objective(sampled_points, objective_values, ax)
...
#     # 回転後のサンプル点を取得してプロット
#     sampled_points = np.array([[- 1, 1] for trial in study.trials])
#     objective_values = np.array([trial.value for trial in study.trials])
#     ax.scatter(sampled_points[:, 0], sampled_points[:, 1], color="red", label="Sampled Points", zorder=5)
